from helper import *
from data_loader_kg import *

# sys.path.append('./')
from model.models import OnlyKGModel
...
class Runner(object):
    def load_data(self):
        ...
        for (org_name,author_group) in OA.items():
            ...
            for each_author in author_group:
                author.add(each_author)
            rel_set.add('org/author/affiliate')
            kbs_tripple.append((sub, rel, obj))
    ...
    def __init__(self, params):
        ...
        self.load_data()
        #这一部分还得改
        self.model = OnlyKGModel(self.node_set_length, self.kb_edge_index, self.kb_edge_type, self.num_rel, self.org_kg_index, self.p)
...
if __name__ == '__main__':
    ...
    # 是否将已知org_cooperate_known中的OO加入kbs，我认为不用加，不然训练集很容易准确率到达100%（知识图谱链接预测中，训练集triples肯定是原有已知图中没有的）

    #超图编码部分
    parser.add_argument('-hyper_model', default = None, help='Graph models for encoding hypergraph')
